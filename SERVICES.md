...
3. The service-specific response is sent to the client:
   - **Simple banner services**: send a protocol-specific banner immediately and close the connection
   - **Multi-step handshake services**: implement full protocol handshakes (VNC, Lightning, Minecraft) to capture client version strings, capabilities, and authentication attempts before closing
...
| 5900 | VNC | Virtual Network Computing | Multi-step RFB 3.8 handshake (protocol version → security types → challenge → auth response → failure) |
...
**Protocol**: RFB (Remote Framebuffer Protocol) — RFC 6143

**Purpose**: webTraffik implements a full multi-step RFB handshake emulator (similar to the Lightning and Minecraft dedicated listeners) that captures client protocol version, security type preferences, and encrypted authentication attempts.

**Handshake Flow**:

1. **Server → Client**: Protocol Version  
   Sends `RFB 003.008\n` — advertises RFB protocol version 3.8 (the most widely supported VNC version)

2. **Client → Server**: Protocol Version  
   Reads 12 bytes of client version string (e.g., `RFB 003.008\n` or `RFB 003.003\n`)

3. **Server → Client**: Security Types  
   Sends a 2-byte security handshake: `\x01\x02` — offers 1 security type (VNC Authentication, type 2)

4. **Client → Server**: Security Type Selection  
   Reads 1 byte indicating the client's chosen security type (typically `\x02` for VNC Authentication)

5. **Server → Client**: Authentication Challenge  
   Sends a 16-byte random challenge generated by `crypto/rand` (this is the DES challenge in real VNC auth)

6. **Client → Server**: Authentication Response  
   Reads 16 bytes of DES-encrypted response (the client's encrypted challenge using their password)

7. **Server → Client**: SecurityResult  
   Sends 4 bytes: `\x00\x00\x00\x01` (authentication failed) — always rejects to encourage brute-force scanners to keep trying

**Why it's convincing**: This is a complete, valid RFB 3.8 handshake that exactly matches what real VNC servers (TigerVNC, TightVNC, RealVNC) perform during connection establishment. Unlike the previous banner-only implementation, this multi-step emulator:
- Captures the client's advertised protocol version (reveals scanner capabilities)
- Records security type preferences (fingerprints VNC client software)
- Obtains the encrypted authentication response (16 bytes of client data that could be analyzed for password cracking attempts)
- Returns an authentication failure that encourages persistent brute-force scanners to continue attempting connections

VNC reconnaissance tools, password crackers, and automated VNC scanners will recognize this as a real VNC server with password authentication enabled.

**Why this port is targeted**: Port 5900 is one of the most heavily-scanned remote access ports:
- **Credential brute-forcing** — VNC authentication is weak (DES-based, limited to 8-character passwords) and heavily targeted by botnet password crackers
- **Remote desktop reconnaissance** — attackers scan for VNC to identify systems with graphical interfaces and potential high-value targets
- **Unpatched VNC exploitation** — numerous VNC server vulnerabilities exist (authentication bypasses, buffer overflows, CVE-2019-15681, CVE-2020-14397)
- **Ransomware delivery** — VNC access provides attackers with GUI access to deploy ransomware interactively
- **IoT device compromise** — many embedded devices and Raspberry Pi systems expose VNC with default or weak passwords
- **Lateral movement** — compromised VNC servers allow attackers to visually navigate internal networks

VNC is a critical remote access target and is constantly probed by both automated scanners and human attackers seeking interactive system access.
